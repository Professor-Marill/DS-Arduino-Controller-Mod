...
void setup() {
 //sets up all of the pins as outputs
 //Sets the voltage of all pins to high which is 3.3V
...
void onDisconnectedController(ControllerPtr ctl) {
  ...
  //Ensures that no buttons remain pressed if and when the controller disconnects by resetting the voltage back to High
  digitalWrite(2, HIGH);
  digitalWrite(3, HIGH);
  digitalWrite(4, HIGH);
  digitalWrite(5, HIGH);
...
//Drops Voltage when the zl button is pressed then returns it to 3.3V
  if (gamepad->l2()){
    digitalWrite(10, LOW);
    //delay(100);
  }
...
//Drops the Voltage when the R button is pressed and then returns it to 3.3V
  if (gamepad->r1()){
    digitalWrite(11, LOW);
   }
   else {
...
//Drops the Voltage when the L button is pressed and then returns it to 3.3V
   if (gamepad->l1()){
    digitalWrite(10, LOW);
   }
   else {
  ...
  //Left Joystick X and Y value variables
  ...
  //Sets the d-pad pins back to high voltage if their values are 0
  if (up == 0) {
    digitalWrite(6, HIGH);
    //delay(100);
  }
  ...
  if (gamepad->dpad()) {
    //changes the value of up to 1 when up is pressed and then lowers the voltage
    if (dpad == 1) {
      up = 1;
      digitalWrite(6, LOW);
      //delay(100);
    }
    //resets up to 0 to reset the voltage back to 3.3V
    else {
      up = 0;
    }
    //changes the value of down to 1 when down is pressed and then lowers the voltage
    if (dpad == 2) {
      down = 1;
      digitalWrite(7, LOW);
      //delay(100);
    }
    //resets down to 0 to reset the voltage back to 3.3V
    else {
      down = 0;
    }
    //changes the value of left to 1 when left is pressed and then lowers the voltage
    if (dpad == 8) {
      left = 1;
      digitalWrite(8, LOW);
      //delay(100);
    }
    //resets left to 0 to reset the voltage back to 3.3V
    else {
      left = 0;
    }
    //changes the value of right to 1 when right is pressed and then lowers the voltage
    if (dpad == 4) {
      right = 1;
      digitalWrite(9, LOW);
      //delay(100);
    }
    //resets right to 0 to reset the voltage back to 3.3V
    else {
      right = 0;
    }
  }
